...
/**
 * bulkDeletePengampuAction - Server Action to remove multiple assignments.
 * @param {string[]} ids - Array of Pengampu IDs
 */
export async function bulkDeletePengampuAction(ids) {
  try {
    const result = await PengampuService.bulkDelete(ids);
    revalidatePath("/dashboard/admin/pengampu");
    return { success: true, data: result };
  } catch (error) {
    console.error("Action Error (bulkDeletePengampu):", error);
