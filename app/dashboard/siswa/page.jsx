      ...
      <div className="group relative flex items-center justify-between min-h-[160px] rounded-[32px] px-10 py-8 bg-indigo border border-indigo-border overflow-hidden shadow-2xl shadow-indigo/20">
        ...
        <div className="relative z-10 flex flex-col gap-2">
          ...
          <p className="text-[13px] text-white/70 font-medium tracking-wide uppercase">
            {user?.kelas ? `Kelas ${user.kelas}` : "Belum Terdaftar Kelas"} · {user?.name || user?.username} · Portal Akademik
          </p>
        </div>
        ...
        <div className="relative z-10 text-right hidden md:block">
          ...
          <div className="bg-white/10 px-4 py-2.5 rounded-2xl backdrop-blur-md border border-white/10 mt-3 inline-block">
            <p className="text-[13px] font-black text-white/90 uppercase tracking-widest leading-none">
          ...
          <Link key={label} href={href}>
            <div className="group bg-surface border border-border rounded-[28px] p-6 flex items-center justify-between hover:border-indigo transition-all cursor-pointer shadow-card hover:shadow-xl hover:-translate-y-1">
              <div className="flex items-center gap-5">
                <div
                  className={`w-14 h-14 rounded-2xl ${bg} flex items-center justify-center transition-transform group-hover:scale-110 duration-300`}
                >
                ...
                <div>
                  ...
                  <p className="text-[11px] font-bold text-ink-3 uppercase tracking-widest mt-2">
                    {label}
                  </p>
                </div>
              </div>
        ...
        <div className="bg-surface border border-border rounded-[32px] shadow-card overflow-hidden">
          <div className="px-8 py-6 border-b border-border bg-cream/30 flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-[13px] font-black text-ink uppercase tracking-wider leading-none">
                  Jadwal Hari Ini
                </h3>
                <p className="text-[10px] font-bold text-ink-3 uppercase tracking-widest mt-1.5">
                  {todayName},{" "}
            ...
            <div className="flex flex-col gap-4">
              {jadwalHariIni.length > 0 ? (
                jadwalHariIni.map((j, i) => (
                  <div
                    key={i}
                    className="group flex items-center gap-6 p-5 rounded-3xl border border-border bg-white hover:border-indigo/30 transition-all"
                  >
                    ...
                    <div className="flex-1">
                      <h4 className="text-[15px] font-black text-ink uppercase tracking-tight mb-1">
                        {j.mapel?.nama || "---"}
                      </h4>
                      <div className="flex items-center gap-3">
                ...
                <div className="py-12 flex flex-col items-center justify-center text-center gap-4 bg-cream/10 rounded-3xl border border-dashed border-border/60">
                  ...
                  <div>
                    <p className="text-[13px] font-black text-ink uppercase">
                      Libur / Tidak Ada Jadwal
                    </p>
                    <p className="text-[11px] font-bold text-ink-3 uppercase tracking-widest mt-1">
                      Gunakan waktu untuk istirahat atau belajar mandiri
                    </p>
                  </div>
                </div>
              )}
        ...
        <div className="flex flex-col gap-8">
          ...
          <div className="bg-surface border border-border rounded-[32px] shadow-card p-8">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-[13px] font-black text-ink uppercase tracking-widest">
                Materi Terbaru
              </h3>
            ...
            <div className="flex flex-col gap-4">
              {data.materi.length > 0 ? (
                  ...
                  <Link key={i} href="/dashboard/siswa/materi">
                    <div className="p-4 rounded-2xl border border-border bg-white hover:border-teal-400/30 transition-all flex items-center gap-4">
                      ...
                      <div className="flex-1 overflow-hidden">
                        <p className="text-[13px] font-black text-ink truncate leading-tight uppercase">
                          {m.judul}
                        </p>
                        <p className="text-[9px] font-bold text-ink-3 uppercase tracking-widest mt-1">
                          {m.mapel.nama}
                        </p>
                      </div>
                    </div>
                ...
                <p className="text-[11px] font-medium text-ink-3 text-center py-6 italic uppercase tracking-widest">
                  Belum ada materi
                </p>
              )}
            </div>
          ...
          <div className="bg-surface border border-border rounded-[32px] shadow-card p-8">
            ...
            <div className="flex flex-col gap-4">
              {data.tugas.length > 0 ? (
                  ...
                  <Link key={i} href="/dashboard/siswa/tugas">
                    <div className="p-5 rounded-[24px] border border-border bg-white hover:border-indigo/30 transition-all">
                      <div className="flex justify-between items-start mb-2">
                        <p className="text-[13px] font-black text-ink tracking-tight uppercase leading-none">
                          {t.judul}
                        </p>
                      </div>
                      <p className="text-[10px] font-bold text-ink-3 uppercase tracking-widest mb-4 opacity-70">
                        {t.mapel.nama}
                      </p>
                ...
                <p className="text-[11px] font-medium text-ink-3 text-center py-8 italic uppercase tracking-widest">
