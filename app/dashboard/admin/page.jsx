      ...
      <div className="relative flex items-center justify-between min-h-[110px] rounded-[32px] px-8 py-7 bg-gradient-to-br from-indigo to-indigo-hover overflow-hidden shadow-2xl shadow-indigo/20 border border-white/10">
        ...
        <div className="relative z-10">
          ...
          <p className="font-black text-3xl text-white tracking-tight">
            Sistem Pusat E-Learning
          </p>
          <p className="text-[13px] text-white/70 font-medium mt-1 uppercase tracking-widest">
            MTS YPI Pulosari Limbangan
          </p>
        </div>
          ...
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <p className="text-[13px] text-white/80 font-bold uppercase tracking-widest mt-1">
                Sistem tidak dapat memproses materi, tugas, dan absensi sebelum
                Tahun Ajaran diaktifkan.
              </p>
            </div>
          </div>
          <Link
            href="/dashboard/admin/tahun-ajaran"
            className="px-8 py-4 bg-white text-red-600 rounded-2xl text-[12px] font-black uppercase tracking-widest hover:bg-neutral-100 transition shadow-lg shrink-0"
          >
            Aktifkan Sekarang
          </Link>
        </div>
        ...
        {stats.map(({ icon: Icon, label, value, color, bg, unit, href }) => (
          <Link key={label} href={href} className="no-underline group">
            <div className="bg-surface border border-border rounded-3xl p-6 flex flex-col gap-4 cursor-pointer hover:border-indigo/30 hover:shadow-2xl hover:shadow-indigo/5 transition-all duration-300 relative overflow-hidden">
              ...
              <div
                className={`w-12 h-12 rounded-2xl ${bg} flex items-center justify-center shadow-inner`}
              >
                <Icon size={22} style={{ color }} />
              </div>

              <div>
                <p className="text-[11px] font-black text-ink-3 uppercase tracking-widest mb-0.5 leading-none">
                  {label}
                </p>
                <div className="flex items-baseline gap-1">
                  <p className="font-black text-3xl text-ink tracking-tighter leading-none">
                    {value}
                  </p>
      ...
      <div className="grid grid-cols-[1fr_340px] gap-6 items-start">
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="bg-surface border border-border rounded-[40px] shadow-card p-8">
            <div className="flex justify-between items-center mb-8">
              <div>
                <h3 className="font-black text-lg text-ink leading-none">
                  Aktivitas Terkini
                </h3>
                <p className="text-[10px] font-bold text-ink-3 uppercase tracking-widest mt-2">
                  Log Sistem Terbaru
                </p>
              </div>
              <button className="px-4 py-2 bg-cream text-ink-3 text-[10px] font-black rounded-xl hover:bg-border transition-colors uppercase tracking-widest">
                Detail
              </button>
            </div>
            ...
            <div className="flex flex-col gap-4">
              {recentActivity.map((a, i) => (
                <div
                  key={i}
                  className={`flex items-center gap-4 p-4 rounded-3xl hover:bg-cream/50 transition-colors ${i !== recentActivity.length - 1
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      <span className="text-[11px] font-bold text-ink-3 leading-none italic">
                        {formatDistance(a.time)}
                      </span>
                    </div>
                    <p className="text-[12px] font-bold text-indigo uppercase tracking-wide">
                      {a.action}{" "}
                      <span className="text-ink-3 font-medium lowercase italic">
                        "{a.target}"
                      </span>
                    </p>
                  </div>
          ...
          <div className="grid grid-cols-2 gap-6">
            <div
              className={cn(
                "border rounded-[32px] p-7 group transition-all",
                studentsNoClass > 0
            ...
            >
              <p
                className={cn(
                  ...
                  studentsNoClass > 0 ? "text-red-700" : "text-indigo-700",
                )}
              >
              ...
              <div className="flex items-center gap-3">
                ...
                <p
                  className={cn(
                    ...
                    studentsNoClass > 0 ? "text-red-950" : "text-indigo-950",
                  )}
                >
                  {studentsNoClass > 0
                    ? `${studentsNoClass} Siswa Tanpa Kelas`
                    : "Data Sinkron (100%)"}
                </p>
              </div>
            </div>
            ...
            <div className="bg-indigo-50/50 border border-indigo-200 rounded-[32px] p-7 group hover:bg-indigo-50 transition-colors">
              <p className="text-[10px] text-indigo-700 font-black uppercase tracking-widest mb-4">
                Storage Used (Supabase)
              </p>
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="bg-surface border border-border rounded-[40px] shadow-card p-8">
            ...
            <p className="text-[10px] font-bold text-ink-3 uppercase tracking-widest mb-6">
              Akses Mudah Data
            </p>
            ...
            <div className="flex flex-col gap-3">
              {[
                ...
                <Link key={a.label} href={a.href} className="group">
                  <div
                    className={`flex items-center gap-4 px-5 py-4 rounded-3xl ${a.bg} border border-transparent hover:border-indigo/20 group-hover:scale-[1.02] transition-all cursor-pointer`}
                  >
          ...
          <div className="bg-indigo-50 border border-indigo-100 rounded-[32px] p-7">
            <div className="flex gap-4">
              ...
              <div>
                <p className="text-[13px] font-black text-indigo-900 leading-none">
                  Auto-Backup
                </p>
                <p className="text-[11px] text-indigo-800/70 mt-2 font-medium leading-relaxed">
                  Backup sistem aktif. Terakhir dicadangkan{" "}
                  <span className="font-bold">3 jam yang lalu</span>.
                </p>
              </div>
            </div>
          </div>
